<?php
/**
 * A model for managing Zelda modules.
 * 
 * @package ZeldaCore
 */
class CMModules extends CObject {
   ...
   * Properties
   */
  private $zeldaCoreModules = array('CZelda', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $zeldaCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  public function ReadAndAnalyse() {
    $src = ZELDA_INSTALL_PATH.'/src';
    if(!$dir = dir($src)) throw new Exception('Mappen kunde inte läsas.');
    $modules = array();
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isZeldaCore']       = in_array($rc->name, $this->zeldaCoreModules);
      $details['isZeldaCMF']        = in_array($rc->name, $this->zeldaCMFModules);
